...
#       limits=c(0,NA)
#     ) +
#     theme(axis.text.x = element_text(angle = -45, hjust=-0.25), aspect.ratio=1/1)
```
...
store_data %>% 
  ...
  ggplot() + 
    ...
    labs(title="Total Sales by Season", y="Total Sales (in millions)") +
    scale_y_continuous(
      labels = scales::number_format(scale = 1/1000000), 
      limits=c(0,NA)
    )
...
store_data_with_holidays %>% 
  ...
  summarize(Total_Sales=mean(Weekly_Sales)) %>% 
  ggplot() + 
    geom_bar(aes(x=factor(Holiday_Flag), y=Total_Sales), stat = "identity") +
    labs(title="Average Sales by Holiday", y="Average Sales (in millions)") +
    scale_y_continuous(
      labels = scales::number_format(scale = 1/1000000), 
      limits=c(0,NA)
    )
